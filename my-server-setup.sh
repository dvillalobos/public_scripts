...
echo "              W E L C O M E                "
echo "                                           "
echo " This is an automatic Server Setup         "
echo " perfect for Data Science & more!          "
echo "                                           "
echo "                 DEVJ                      "
echo "      Duubar E. Villalobos Jimenez         "
echo "              Data Scientist               "
echo "                                           "
echo "            mydvtech@gmail.com             "
echo "                                           "
echo "                                           "
...
# https://pandoc.org/
sudo apt-fast -y install pandoc
...
# Hydrogen - a Jupyter-like extension that lets us execute code interactively.
# Results of code appear inline as we execute them and do not clutter up version control
# or modify the file in any way (just like R notebooks!). The name is actually a pun
# - Hydrogen is the primary component of the planet Jupiter (Jupyter is the tool that Hydrogen wraps).
...
# Need to remove duplicates in order to avoid problems
...
sudo apt-fast -y install openssl
...
# Shiny Server is now installed and running. Assuming there were no problems,
# if you go to http://123.456.1.2:3838/ you should see Shiny Server’s default homepage,
# which includes some instructions and two Shiny apps.


# R Essential Packages

sudo su - -c "R -e \"install.packages(c('repr', 'IRdisplay', 'evaluate', 'crayon', 'pbdZMQ', 'devtools', 'uuid', 'digest'))\""
sudo su - -c "R -e \"install.packages(c('tidyverse', 'caret', 'nnet'))\""

# In order to install one by one
...
# Why? Because if you log into R and install packages, by default they will be installed in your personal library and will only
# be accessible to the current user (dean in this case). By running the command the way I do above, it installs the packages as
# the root user, which means the packages will be installed in a global library and will be available to all users.
...
# Therefore, we edit /etc/mysql/mariadb.conf.d/50-server.cnf and comment out the line bind-address = 127.0.0.1:
# $ sudo nano /etc/mysql/mariadb.conf.d/50-server.cnf
  
# MariaDB setup will be:
# user: root
# root password: MariaDBRootPassword
...
New_York\n
New_York\n
Data_Science_Organization\n
Data_Science_Department\n
...
# To automatically select the best option
